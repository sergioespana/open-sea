...
const createFormElement = ({ help, label, type, ...otherProps }) => {
	const id = slug(`id-${label}`).toLowerCase();
	const props = { ...otherProps, id };

	switch (type) {
		case 'checkbox': return (
			<Wrapper>
				<InputLabel component="legend">{ label }</InputLabel>
				<Checkbox help={help} {...props} />
			</Wrapper>
		);
...
const Input = styled(({ className, ...props }) => (
	...
	& > p {
		...
		font-size: 0.75rem;
		color: #707070;
		padding: 0 0 0 ${({ disabled }) => disabled ? 5 : 0}px;
	}
`;
